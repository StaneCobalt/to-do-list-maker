...
function checkedBox(c) {
	...
	var fader = setInterval(function() {
		if(!chk.style.opacity)
			chk.style.opacity = 1;
		if(!lbl.style.opacity)
			lbl.style.opacity = 1;
		if (chk.style.opacity < 0.1)
			clearInterval(fader);
		else {
...
function resetBackgroundSize() {
	// w = window.innerWidth;
	h = (window.scrollHeight >= window.innerHeight) ? window.scrollHeight : window.innerHeight;
	w = (h*1.5 >= window.innerWidth) ? h*1.5 : window.innerWidth;
	var size = w.toString() + "px " + h.toString() + "px";
